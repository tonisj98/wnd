<template>
  ...
  <div class="flex flex-wrap">
    <div class="p-8 ml-10 sm:ml-20 sm:mx-4  w-100 relative" v-for="(product, index) in filteredBestsellers" :key="index">
      <router-link :to="{ name: 'ProductDetail', params: { id: product.id } }">
        <div class="ml-20">
          <Card :product="product" :allProducts="true" :users="users" />
        </div>
      </router-link>
    </div>
  </div>
  ...
  <div v-if="bestsellers.length == 0" class="flex items-center justify-center h-screen text-xl">
<p> {{ $t('noProducts') }}</p>
</div>
...
this.bestsellers =productStore.favouriteProducts;
